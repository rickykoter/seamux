...
```
  ✗ a deep-plan is awaiting its alignment check — sync-order-status-oms-to-erp (0/3 increments done)
    finish it, or if it is abandoned remove: …/deep-plan/active/sync-… …/deep-plan/state/sync-….json
  ✗ a session here is in plan mode: handle-status-change (workspace:19)
